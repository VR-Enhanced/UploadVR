...
ObjectControls.prototype._hoverOut = function(object) {
  ...
  this.hoverOut();
  console.log(this.intersectedPoint)
...
ObjectControls.prototype.checkForIntersections = function() {
  ...
  if(G.pointerLock){
    this.raycaster.set(G.controls.getObject().position,  G.controls.getDirection());
  } else {
    var lookAt = new THREE.Vector3(0,0,-1);
    lookAt.applyQuaternion( this.eye.quaternion );
    this.raycaster.set(this.eye.position,  lookAt);
  }
...
ObjectControls.prototype._objectIntersected = function(intersected) {
  ...
  this.intersectedPoint = intersected[0].point;
  if (!this.intersected) {
...
ObjectControls.prototype._noObjectIntersected = function() {
  ...
  if (this.intersected) {
    this.lastPointIntersected = this.intersected.point;
    this._hoverOut(this.intersected);
